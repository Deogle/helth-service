...
type GetActivityLogsParams = RequireAtLeastOne<
  ActivityLogsParams,
...
const FitbitApi = (tokens: { access_token: string; refresh_token: string }) => {
  ...
  const fetchUser = async (userId?: string) => {
    const response = await instance.get<{ user: FitbitUser }>(
  ...
  const fetchHrData = async (
    date: string,
    period: FitbitHeartRatePeriod = "1d",
    userId?: string
  ) => {
    const response = await instance.get(
  ...
  const clearSubscriptions = async () => {
    const response = await instance.get<FitbitServiceCollection>(
      `/user/-/apiSubscriptions.json`
    );
    const { apiSubscriptions } = response.data;

    const collectionTypes = apiSubscriptions.map((sub) => ({
      type: sub.collectionType,
      id: sub.subscriptionId,
    }));
    return Promise.all(
      collectionTypes.map(({ type, id }) => {
  ...
  const createSubscription = async (id: string, type: string) => {
    const url = `/user/-/${type}/apiSubscriptions/${getSubscriptionId(
      id,
      type
    )}`;
    try {
  ...
  const fetchActivities = async (params: GetActivityLogsParams) => {
    const defaultActivityLogsParams: Omit<
    ...
    const response = await instance.get<FitbitActivityResponse>(
      `/user/-/activities/list.json`,
      { params: { ...defaultActivityLogsParams, ...params } }
    );
    return response.data;
  };
...
const FitbitService = (tokens: {
  ...
  const subscribe = async () => {
    ...
    const results = await Promise.allSettled(
      collectionTypes.map((type) =>
        fitbitApi.createSubscription(user.encodedId, type)
      )
    );
    logger.info("Finished creating subscriptions", {
      user: user.encodedId,
      results: results.map((pResult) => pResult.status),
    });
  };
  const getSubscriptions = async () => {
    const fitbitApi = FitbitApi(tokens);
  ...
  const getActivityDistance = (
    distance: number | undefined,
    distanceUnit: FitbitDistanceUnit | undefined
  ) => {
